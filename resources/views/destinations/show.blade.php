...
@section('title', $destination->meta_title ?? $destination->labelAr())
...
<div class="dest-hero">
    <div class="dest-route">
        <span class="ar">{{ $destination->from_name_ar }}</span>
        <span class="en">{{ $destination->from_name_en }}</span>
        <span class="arrow">✈</span>
        <span class="ar">{{ $destination->to_name_ar }}</span>
        <span class="en">{{ $destination->to_name_en }}</span>
    </div>
    <div class="dest-tags">
        <span class="dest-tag">✈ {{ $destination->flight_duration }}</span>
        <span class="dest-tag ar">🗓 {{ $destination->best_time_ar }}</span>
        <span class="dest-tag en">🗓 {{ $destination->best_time_en }}</span>
        <span class="dest-tag">{{ $destination->from_code }} → {{ $destination->to_code }}</span>
    </div>
    <a href="/results?origin={{ $destination->from_code }}&destination={{ $destination->to_code }}&date={{ now()->addWeek()->format('Y-m-d') }}&adults=1"
       class="dest-cta-btn">
...
<div class="page-content">
    <div class="dest-info">
        <div class="info-card">
            ...
            <div class="info-value">{{ $destination->flight_duration }}</div>
        </div>
        <div class="info-card">
            <div class="info-icon">📅</div>
            <div class="info-label ar">أفضل وقت للزيارة</div>
            <div class="info-label en">Best Time to Visit</div>
            <div class="info-value ar">{{ $destination->best_time_ar }}</div>
            <div class="info-value en">{{ $destination->best_time_en }}</div>
        </div>
        <div class="info-card">
            <div class="info-icon">🛫</div>
            <div class="info-label ar">رمز المطار</div>
            <div class="info-label en">Airport Codes</div>
            <div class="info-value">{{ $destination->from_code }} → {{ $destination->to_code }}</div>
        </div>
    </div>

    <div class="desc-section">
        <p class="ar">{{ $destination->description_ar }}</p>
        <p class="en">{{ $destination->description_en }}</p>
    </div>
